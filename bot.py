...
logging.basicConfig(
    ...
    level=logging.INFO,
    filename='bot.log'  # إذا أردت رؤية الأخطاء في الشاشة فقط احذف هذا السطر
)
...
async def backup_database():
    try:
        ...
        session = get_session()
        try:
            backup = Backup(filename=backup_file, size=os.path.getsize(backup_file))
            session.add(backup)
            session.commit()
        finally:
            session.close()
...
async def get_analytics_report(days=7):
    ...
    try:
        ...
        posts_count = session.query(Analytics).filter(Analytics.action == 'post', Analytics.date >= start_date).count()
        new_users = session.query(User).filter(User.join_date >= start_date).count()
        new_content = session.query(Content).filter(Content.added_at >= start_date).count()
        notifications_sent = session.query(Analytics).filter(Analytics.action == 'notification_sent', Analytics.date >= start_date).count()
        
        channel_stats = session.query(Channel.title, func.count(Analytics.id).label('posts_count')).join(Analytics, Channel.channel_id == Analytics.channel_id).filter(Analytics.action == 'post', Analytics.date >= start_date).group_by(Channel.title).all()
        category_stats = session.query(Content.category, func.count(Content.id).label('content_count')).filter(Content.added_at >= start_date).group_by(Content.category).all()
        
        return {'period': f'آخر {days} أيام', 'posts': posts_count, 'new_users': new_users, 'new_content': new_content, 'notifications': notifications_sent, 'channel_stats': channel_stats, 'category_stats': category_stats}
    finally:
        session.close()
...
async def search_content(query, category=None, limit=10):
    ...
    try:
        ...
        search_query = search_query.filter(Content.text.contains(query))
...
async def schedule_content_posting():
    ...
    try:
        ...
        if content:
            task_queue.add_task(lambda: post_content_to_channels(content), priority=1, delay=random.randint(60, 3600))
    finally:
        session.close()
    ...
    try:
        ...
        for channel in channels:
            try:
                ...
                if channel.msg_format == 'blockquote': text = f"<blockquote>{text}</blockquote>"
                
                await APPLICATION.bot.send_message(channel.channel_id, text, parse_mode='HTML')
                
                analytics = Analytics(action='post', channel_id=channel.channel_id, content_id=content.id, meta_data=json.dumps({'channel_title': channel.title}))
...
def get_emoji_category_icon(category):
    emoji_map = {
        'حب': '💕', 'عيد ميلاد': '🎂', 'اقتباسات عامة': '💭', 'ابيات شعرية': '📜',
        'ديني': '🙏', 'مضحك': '😂', 'عام': '📋', 'تقني': '💻', 'رياضة': '⚽',
        'فن': '🎨', 'سياسة': '🏛️', 'اقتصاد': '💰'
    }
...
# --- معالجة الرسائل (الجديد) ---
# هذا المعالج ضروري لاستقبال رابط القناة أو النصوص اليدوية

async def document_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    mode = context.user_data.get('mode')
    if mode and mode == 'upload_file':
        ...
        for line in lines:
            if line.strip():
                new_content = Content(category=category, text=line.strip(), added_by=update.effective_user.id)
                session.add(new_content)
                added_count += 1
        session.commit()
        session.close()
        await update.message.reply_text(f"✅ تم رفع {added_count} نص بنجاح!")
        context.user_data['mode'] = None
        context.user_data['temp_category'] = None
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # وضع إضافة قناة: استقبال الرابط
    if mode == 'set_req_channel':
        session = get_session()
        setting = session.query(BotSettings).filter_by(key='required_channel').first()
        if setting:
            setting.value = text
        else:
            session.add(BotSettings(key='required_channel', value=text))
        session.commit()
        session.close()
        await update.message.reply_text(f"✅ تم ضبط القناة الإجبارية: {text}")
        context.user_data['mode'] = None

    elif mode == 'upload_manual':
        category = context.user_data.get('temp_category')
        session = get_session()
        new_content = Content(category=category, text=text, added_by=user_id)
        session.add(new_content)
        session.commit()
        session.close()
    ...
    elif mode == 'add_channel_link':
        if not text.startswith('@'):
            await update.message.reply_text("❌ الرابط يجب أن يبدأ بـ @ (مثال: @my_channel)")
            return
        
        # حفظ الرابط المؤقت وطلب الفئة
        context.user_data['temp_channel_link'] = text
        context.user_data['mode'] = 'add_channel_category'
        
        buttons = [[InlineKeyboardButton(name, callback_data=f"set_cat_{code}")] for name, code in CATEGORIES]
        buttons.append([InlineKeyboardButton("🔙 رجوع", callback_data="cancel_add_channel")])
        await update.message.reply_text("اختر فئة القناة:", reply_markup=InlineKeyboardMarkup(buttons))
    ...
    # وضع رفع محتوى يدوي
        ...
        if category:
            ...
            try:
                ...
                session.add(new_content)
                session.commit()
                await update.message.reply_text("✅ تمت إضافة النص بنجاح!")
                context.user_data['mode'] = None
                context.user_data['temp_category'] = None
            except Exception as e:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التعامل مع وضع "إلغاء إضافة القناة"
    if data == "cancel_add_channel":
        context.user_data['mode'] = None
        context.user_data['temp_channel_link'] = None
        await return_to_main_menu(update, context, query)
        return

    # التعامل مع إتمام إضافة القناة (اختيار الفئة)
    if data.startswith("set_cat_"):
        if context.user_data.get('mode') == 'add_channel_category':
            ...
            try:
                ...
                try:
                    ...
                    exists = session.query(Channel).filter_by(channel_id=chat_id).first()
                    if exists:
                        await query.edit_message_text("⚠️ هذه القناة مضافة بالفعل!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data=f"back_{current_role}")]]))
                        context.user_data['mode'] = None
                        context.user_data['temp_channel_link'] = None
                        return
                    ...
                    analytics = Analytics(
                        ...
                        channel_id=chat_id,
                        user_id=user_id,
                        meta_data=json.dumps({'category': category})
                    )
                    session.add(analytics)
                    session.commit()
                    
                    await query.edit_message_text(f"✅ تمت إضافة القناة: {chat_title}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data=f"back_{current_role}")]]))
                    
                except Exception as e:
                    logger.error(f"Error adding channel: {e}")
                    await query.edit_message_text(f"❌ حدث خطأ: {e}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data=f"back_{current_role}")]]))
                finally:
                    session.close()
                    context.user_data['mode'] = None
                    context.user_data['temp_channel_link'] = None
                ...
                logger.error(f"Critical error in channel setup: {e}")
                await query.edit_message_text(f"❌ خطأ في جلب بيانات القناة.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data=f"back_{current_role}")]]))
                context.user_data['mode'] = None
                return
    ...
    if data.startswith("back_"):
        ...
        if target_role == "admin" and current_role == "dev": target_role = "dev"
        elif target_role == "user" and current_role in ["admin", "dev"]: target_role = current_role
        
        # تنظيف الوضع عند الرجوع
    ...
    if data == "add_channel_start":
        ...
        await query.edit_message_text("🔗 أرسل رابط القناة الآن (مثال: @my_channel):", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data=f"back_{current_role}")]]))
    
    elif data == "set_req_channel":
        context.user_data['mode'] = 'set_req_channel'
        await query.edit_message_text("🔗 أرسل رابط القناة الإجبارية:")

    elif data == "toggle_status":
        session = get_session()
        status = session.query(BotSettings).filter_by(key='global_status').first()
        if status:
            status.value = 'off' if status.value == 'on' else 'on'
        else:
            session.add(BotSettings(key='global_status', value='off'))
        session.commit()
        session.close()
        await query.edit_message_text("✅ تم تغيير حالة البوت.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="bot_settings")]]))
    ...
    if current_role in ["admin", "dev"]:
        if data == "stats":
            ...
            session = get_session()
        req_ch = session.query(BotSettings).filter_by(key='required_channel').first()
        status = session.query(BotSettings).filter_by(key='global_status').first()
        session.close()
        text = f"""⚙️ <b>إعدادات البوت:</b>

📢 القناة الإجبارية: {req_ch.value if req_ch else 'غير محددة'}
🚦 حالة البوت: {'نشط' if not status or status.value == 'on' else 'متوقف'} """
        btns = [
            [InlineKeyboardButton("📢 ضبط القناة الإجبارية", callback_data="set_req_channel")],
            [InlineKeyboardButton("🚦 تغيير حالة البوت", callback_data="toggle_status")],
    ...
    if current_role == "user":
        if data == "user_random":
            ...
            session = get_session()
        user = session.query(User).filter_by(user_id=user_id).first()
        text = f"""⚙️ <b>إعداداتك:</b>

🎨 المظهر الحالي: {user.theme}
🌐 اللغة الحالية: {user.preferred_language}"""
        btns = [
            [InlineKeyboardButton("🎨 تغيير المظهر", callback_data="change_theme")],
...
async def show_notifications_menu(query, role):
    ...
    try:
        ...
        if not notifications:
            await query.edit_message_text("🔔 لا توجد إشعارات مجدولة حالياً.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data=f"back_{role}")]]))
            return
...
def main():
    ...
    # إضافة معالج الرسائل (هام جداً لاستقبال الروابط والنصوص)
    # استخدمنا TEXT & ~filters.COMMAND لتجنب تداخل الأوامر
